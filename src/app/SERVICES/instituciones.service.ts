...
function sort(countries:Instituciones[], column:SortColumn, direction:string):Instituciones[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: Instituciones, term: string, pipe: PipeTransform) {
  return country.nombre.toLowerCase().includes(term.toLowerCase())
          || pipe.transform(country.id_tipo_institucion).includes(term);
}
...
export class InstitucionesService {
  public url='api/instituciones';
  public urli='api/tipo_institucion';

  private _loading$ = new BehaviorSubject<boolean>(true);
  public _search$ = new Subject<void>();
  private _countries$ = new BehaviorSubject<Instituciones[]>([]);
  private _total$ = new BehaviorSubject<number>(0);
  public PERSONAS:Instituciones[]=<any>[];
  ...
  this.getEquipos().subscribe(
    res => {
      ...
      //console.log('Este es el listado de las'+this.PERSONAS);
    }
    );
  }
