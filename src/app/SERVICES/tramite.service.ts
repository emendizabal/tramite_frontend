...
import { Tramite } from '../COMPONENTES/tramite/tramite';
...
import { SortColumn, SortDirection } from '../COMPONENTES/tramite/tramite.directive';
...
function sort(countries:Tramite[], column:SortColumn, direction:string):Tramite[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: Tramite, term: string, pipe: PipeTransform) {
  return country.descripcion_tramite.toLowerCase().includes(term.toLowerCase())
          || pipe.transform(country.costo_tramite).includes(term);
}
...
export class TramiteService {
  public url='api/tramites';
  public urli='api/tipo_institucion';

  private _loading$ = new BehaviorSubject<boolean>(true);
  public _search$ = new Subject<void>();
  private _countries$ = new BehaviorSubject<Tramite[]>([]);
  private _total$ = new BehaviorSubject<number>(0);
  public PERSONAS:Tramite[]=<any>[];
  ...
  listarEquipo() {
    this.getEquipos().subscribe(
      res => {
        ...
        //console.log('El listado de Tramites es el siguiente'+this.PERSONAS);
      }
      );
    }
    ...
    getTramiteArea()
    {
      let url='api/tramite_area';
      return this.http.get(url);
    }
    getUnTramiteArea(id:number){
      let url='api/tramite_area';
      return this.http.get<any>(url+'/'+id);
    } 

    getUnTramiteRequisito(id:number){
      let url='api/tramite_requisito';
      return this.http.get<any>(url+'/'+id);
    } 
...
addTramiteRequisito(equipo:any){
  let url='api/tramite_requisito/';
  return this.http.post(url,equipo)
  .pipe(
    tap(() =>{
